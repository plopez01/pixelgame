...
void draw(){
  ...
  if(currentime-lastime[1] > 50){
    ...
    switch(phase){
        ...
        try{
          for(int i = 0; i<objects.length; i++){
            if(objects[i] != null){
                  ...
                  objects[i].sizex = int(random(10, size));
                  objects[i].sizey = int(random(10, size));
                objects[i].pxcolor = color(int(random(0, 255)), int(random(0, 255)), int(random(0, 255)));
            }
      ...
      try{
          for(int i = 0; i<objects.length; i++){
            if(objects[i] != null){
              objects[i].sizex = int(random(size, size*2));
              objects[i].sizey = int(random(size, size*2));
            }
      ...
      size = 0;
      if(warnlen < 47){
          warnpos[warnlen] = int(random(height));
          warnlen++;
        }else{
  ...
  if(currentime-lastime[0] > rate){
    ...
    if(objectnum > 490){
     ...
     if(maindirection < 2){
       objects[objectnum] = new Pixel(int(random(constrain[0], constrain[1])), spawnpoint, vel, size, size, color(pcolorR,pcolorB,pcolorG), true, maindirection, rgbsplit);
     }else{
       objects[objectnum] = new Pixel(spawnpoint, int(random(constrain[0], constrain[1])), vel, size, size, color(pcolorR,pcolorB,pcolorG), true, maindirection, rgbsplit); 
     }
...
class Pixel { 
  ...
  Pixel (int x, int y, int m, int sx, int sy, color c, boolean g, int f, boolean rgb) {  
    xpos = x;
    ypos = y;
    mass = m;
    pxcolor = c;
